...
reg.pred <- predict(reg, test.df, type = "response")
# first six values 
head(reg.pred)
```
...
## 9 confusion matrix 
```{r}
confusionMatrix(as.factor(ifelse(reg.pred > 0.5, 1, 0)), as.factor(test.df$Competitive.),positive = "1")
...
# gain table 
library(gains)
gain <- gains(test.df$Competitive., reg.pred, groups=10)
